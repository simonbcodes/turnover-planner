...
for type in computer_types: # filter by classes in Stern
...
for type in computer_types: # sort the classes by the number of students in the class within computer type (non-Stern)
...
pairings = check_perfect_matches(pairings, old_grouped_by_type, new_grouped_by_type) # do 2 sets of perfect matches, Stern and non-Stern
pairings = check_perfect_matches(pairings, stern_old_by_type, stern_new_by_type)

pairings, leftovers_old, leftovers_new = check_matches(pairings, old_grouped_by_type, new_grouped_by_type) # check matches for Stern and non-Stern locations
pairings, stern_leftovers_old, stern_leftovers_new = check_matches(pairings, stern_old_by_type, stern_new_by_type)

for type in computer_types: # combine leftovers
    leftovers_new[type] = leftovers_new[type].append(stern_leftovers_new[type], sort=False)
    leftovers_old[type] = leftovers_old[type].append(stern_leftovers_old[type], sort=False)
